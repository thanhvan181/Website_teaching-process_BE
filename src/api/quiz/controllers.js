...
const getQuiz = async (req, res, next) => {
  try {
    ...
    const quiz = await quizService.find(filter);
...
const updateQuiz = async (req, res) => {
  ...
  const {name, backgroundImage, description, pointsPerQuestion, isPublic, tags, questionList} =
    req.body;
  const quiz = new Quiz({
    _id: id,
    name,
...
const deleteQuestion = async (req, res) => {
  ...
  try {
    let questionIndex = quiz.questionList.findIndex((obj) => obj._id == questionId);
...
const likeQuiz = async (req, res) => {
  ...
  try {
    ...
    if (index === -1) {
      ...
      quiz.likesCount = quiz.likesCount.filter((id) => id !== String(req.user.id));
    }
    const updatedQuiz = await Quiz.findByIdAndUpdate(id, quiz, {new: true});
    res.json(updatedQuiz);
  } catch (error) {
